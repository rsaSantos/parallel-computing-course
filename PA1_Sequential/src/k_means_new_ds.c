...
void calculate_centroids()
{
    ...
    for (i = 0; i < NCLUSTERS; i++)
    {
        clusters_center->x[i] = 0;
        clusters_center->y[i] = 0;
    }
    ...
    for (i = 0; i < NPOINTS; i++)
    {
        ...
        clusters_center->x[cluster] += _points_->x[i];
        clusters_center->y[cluster] += _points_->y[i];
    }
    ...
    for (i = 0; i < NCLUSTERS; i++)
    {
        ...
        clusters_center->x[i] /= new_clusters_size[i];
        clusters_center->y[i] /= new_clusters_size[i];
    }
}
...
void print_results(int iterations)
{
    ...
    for (i = 0; i < NCLUSTERS; i++)
    {
        printf("Center: (%.3f, %.3f) : Size: %d\n", clusters_center->x[i], clusters_center->y[i], new_clusters_size[i]);
    }
