...
#pragma omp parallel
    {
        float *new_x_private = (float *)malloc(n_clusters * sizeof(float));
        float *new_y_private = (float *)malloc(n_clusters * sizeof(float));
        int *new_private = (int *)malloc(n_clusters * sizeof(int));
...
#pragma omp for schedule(static)
        for (int i = 0; i < n_points; i++)
        {
            ...
            new_private[cluster]++;
            //
            new_x_private[cluster] += _points_->x[i];
            new_y_private[cluster] += _points_->y[i];
        }
...
#pragma omp critical
        {
            for (int i = 0; i < n_clusters; i++)
            {
                clusters_center->new[i] += new_private[i];
                clusters_center->new_x[i] += new_x_private[i];
                clusters_center->new_y[i] += new_y_private[i];
            }
        }
        free(new_x_private);
        free(new_y_private);
...
int k_means(int n_points, int n_clusters)
{
    ...
    //
    // TODO: Criar função apenas para ver tamanho dos clusters.
    // Os centros já foram calculados.
    calculate_centroids(n_points, n_clusters);
